...
def invert(image, network='alexnet', size=224, layer='features.4', alpha=6, beta=2,
    ...
    # print('\nimage size: ', img_.size())
    print('\nModel Name: {}'.format(network))
    print('\nArchitecture:\n{}'.format(model))
    ...
    learning_rate = 1.2
    # optimizer_orig = torch.optim.Adam([x_], lr=learning_rate)  ##adam_lr=0.7

    from sam import SAM
    base_optimizer = torch.optim.Adam
    optimizer = SAM([x_], base_optimizer, lr=learning_rate)
    ...
    for a in ax:
        a.set_xticks([])
        a.set_yticks([])
    plt.savefig('saved_results/alexnet_Adam_SAM_{}.png'.format(layer), bbox_inches='tight')
    plt.show()
...
if __name__ == '__main__':
    ...
    try:
        ...
        parser.add_argument('--image', type=str,
                default='images_to_invert/celeba_000010.png')
        parser.add_argument('--network', type=str, default='alexnet')  #vit_l_16
        parser.add_argument('--size', type=int, default=227)
        parser.add_argument('--layer', type=str, default='features.2') #encoder.layers.encoder_layer_4
