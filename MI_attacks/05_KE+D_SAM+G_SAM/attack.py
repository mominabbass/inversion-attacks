...
def dist_inversion(G, D, T, E, iden, itr, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1,
    ...
    solver = SAM(params, base_optimizer, lr=0.05, adaptive=True)  # 0.02, 0.1, 0.001, 0.0085, 0.01, 0.05, 0.06
    ...
    for i in range(iter_times):
        ...
        Total_Loss.backward()
        solver.first_step(zero_grad=True)
        ...
        Total_Loss.backward()
        solver.second_step(zero_grad=True)
    ...
    for random_seed in range(num_seeds):
        ...
        for i in range(bs):
            ...
            save_tensor_images(sample.detach(),
                               os.path.join(save_img_dir, "attack_iden_{}_{}.png".format(gt + 1, random_seed)))
            ...
            if eval_iden[i].item() == gt:
                ...
                save_tensor_images(best_img.detach(), os.path.join(success_dir,
                                                                   "{}_attack_iden_{}_{}.png".format(itr, gt + 1,
                                                                                                     int(no[i]))))
                no[i] += 1
