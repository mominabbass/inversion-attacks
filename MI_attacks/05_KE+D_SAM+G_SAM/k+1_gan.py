...
if __name__ == "__main__":
    ...
    if model_name_T.startswith("VGG16"):
        T = VGG16(1000)
        path_T = './target_model/target_ckp/VGG16_87.33_allclass.tar' ##VGG16_88.26.tar
    ...
    dg_optimizer = SAM(DG.parameters(), base_optimizer, lr=0.001, adaptive=True)  ##tried: 0.1, 0.001, 0.01, 0.0001, 0.0005

    g_optimizer = SAM(G.parameters(), base_optimizer, lr=0.001, adaptive=True)  ##tried:
    ...
    for epoch in range(epochs):
        ...
        for i, imgs in enumerate(dataloader):
            ...
            dg_loss.backward()
            dg_optimizer.first_step(zero_grad=True)
            ...
            dg_loss = loss_lab + loss_unlab
            dg_loss.backward()
            dg_optimizer.second_step(zero_grad=True)
            ...
            if step % n_critic == 0:
                ...
                g_loss.backward()
                g_optimizer.first_step(zero_grad=True)
                ...
                g_loss = torch.mean((mom_gen - mom_unlabel).abs()) + 1e-4 * Hloss
                g_loss.backward()
                g_optimizer.second_step(zero_grad=True)
